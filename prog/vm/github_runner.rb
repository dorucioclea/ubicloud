...
class Prog::Vm::GithubRunner < Prog::Base
  ...
  def pick_vm
    ...
    boot_image, location_id, size = if label_data["arch"] == "x64" && label_data["boot_image"] == "github-ubuntu-2204" && github_runner.installation.project.get_ff_aws_alien_runners
      [Config.send(:"#{label_data["boot_image"].tr("-", "_")}_aws_ami_version"), Config.github_runner_aws_location_id, Option.aws_instance_type_name("m7a", label_data["vcpus"])]
    else
      [label_data["boot_image"], Location::GITHUB_RUNNERS_ID, label_data["vm_size"]]
    end
